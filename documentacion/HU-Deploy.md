...
* **Herramientas:** GitHub Actions, Node.js 20, ESLint, Vitest, Docker Buildx, AWS EC2, Appleboy SSH Action, Docker Compose.
* **Estrategia de Despliegue:** Recreación de contenedor sin afectar dependencias del backend (`--no-deps`).
...
* **Capa de Arquitectura:** Infraestructura (CI/CD Pipeline)
* **Herramientas:** GitHub Actions, JDK 17 (Temurin), Maven, Testcontainers, Docker Buildx, AWS EC2, Appleboy SSH.
* **Estrategia de Despliegue:** Caché global de Maven y reinicio simultáneo `--no-deps` mediante docker-compose.
